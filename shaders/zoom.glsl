...
void mainImage(out vec4 O, vec2 C) {
  float
    ...
    ;
  vec4
      o = vec4(0)     // Accumulated color/lighting
    , p     // Current 3D position along ray
    , t     // Temporary variable for intermediate calculations
    ;
  vec2 r = iResolution.xy;  // Screen resolution

  // WebGL 1.0 compliant for loop
  for(int i = 0; i < 77; i++)
      // Convert 2D pixel to 3D ray direction
      p = vec4(z*normalize(vec3(C-.5*r,r.y)),.1*iTime*0.3)
    ...
    , p.xy *= mat2(cos(2.+t.z+vec4(0,11,33,0)))
    ...
    , t = (1.+sin(.5*t.z+length(p-t)+vec4(0,4,3,6)))
       / (.5+2.*dot(t.xy,t.xy))
