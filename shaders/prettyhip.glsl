...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  float canim=smoothstep(-.2,.2,sin(iTime*0.5));
  ...
  if(march.isect.x<1E-2){
    ...
    if(mtl==0.){
      ...
      float lv=0.5 + 0.5*sin(march.grid.h.z*3. + iTime*0.5); // Procedural animation instead of audio
      ...
      led*=vec3(2,1,2);
      float plasma=sin(length(c)*10.-3.*iTime+march.grid.h.z*.7);
      plasma+=sin(c.y*10.-2.*iTime);
      led*=.5+.5*sin(plasma);
      col+=2.*led;
      basecol=.04*led;
    }else if(mtl==5.){//808
