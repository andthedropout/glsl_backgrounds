/*
    ...
    Note: Modified to work without texture inputs (iChannel0)
*/
...
// Simple procedural texture replacement for iChannel0
...
void mainImage( out vec4 O, in vec2 F )
{
    ...
    // tile background (replaced texture with procedural)
    float d = max(abs(p.x)*.866025 + abs(p.y)/2., abs(p.y))-(.5-ln);
    C = mix(vec3(.0125),procTexture(p*2.)*vec3(0.906,0.282,0.075),smoothstep(px,-px,d) );
